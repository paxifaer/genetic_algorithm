...
int VariateMasker(std::vector<int> champion, int fitness_standard, int fitness_max)//基因变异概率
{
...
void Variation(std::vector<int> &individual, int fitness_standard, int fitness_max)//基因变异
{
...
void CrossingOver(std::vector<int> & chromosome_x, std::vector<int> & chromosome_y, int &num, int fitness_standard, int fitness_max) {//染色体交叉互换
    int n = num;
    int nu = 0;
...
void GeneticRecombination(std::vector<std::vector<int>> champion, int fitness_standard, int i, int fitness_max)//开始基因重组
{
    int num = 1;
    int nu = 0;
    for (int i = 1; i <= 5; i++) {
